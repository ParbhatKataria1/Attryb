...
function DrawerLogin() {
  ...
  const { mode, setmode, dark, light } = useContext(LightmodeContext);
    ...
    <>
      ...
      >
        ...
        <DrawerContent bg={mode ? dark : "white"}>
          <DrawerCloseButton color={mode ? "white" : dark} />
...
function LoginOption() {
  const { mode, setmode } = useContext(LightmodeContext);
  return (
    ...
    <>
      <Flex  alignItems={'center'} colorScheme="red" onClick={onOpen}>
        <svg
          ...
          height="24"
          viewBox="0 0 24 24"
          fill={mode ? "white" : dark}
        >
          <path d="M16 10v-5l8 7-8 7v-5h-8v-4h8zm-16-8v20h14v-2h-12v-16h12v-2h-14z" />
        </svg>

        <Text display={value?'block':'none'} color={mode ? "white" : dark} ml={"10px"}>
          Logout
        </Text>
      </Flex>
          ...
          <AlertDialogContent color={mode?'white':'black'} bg={mode?dark:'white'}>
            ...
            <AlertDialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  toast({
                    title:`${sessionStorage.getItem("username")?'Logged Out':'Error'}` ,
                    status: `${sessionStorage.getItem("username")?'info':'error'}`,
                    duration: 9000,
                    isClosable: true,
                    position:'bottom-left',
                    description: `${sessionStorage.getItem("username")?'You have successfully logged out.':'You are not signed in'}`
                  })
